...
globalMetadataFiles      | Specify a list of JSON file path containing globalMetadata settings, as similar to `{"key":"value"}`. It will merge with the `globalMetadataFiles` from the config file and `globalMetadataFiles` from command line. If the same key for global metadata was set, the order to be overriden would be: `global metadata from docfx config file` < `global metadata from global metadata files` < `global metadata from command line` 
