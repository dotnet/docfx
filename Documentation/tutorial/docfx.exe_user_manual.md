...
rootTocPath        | The Root TOC Path used for navbar in current version, relative to output root. If not set, will use the TOC in output root in current version if exists. It's useful for a version to specify a navbar TOC no matther whether it's within the same version.
