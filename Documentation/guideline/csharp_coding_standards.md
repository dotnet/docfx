...
Introduction
----------------
The coding C# Coding Standards will be used in conjunction with customized versions of *StyleCop* and *FxCop* [**TODO**] during both development and build process. This will help ensure that all developers on the team in a consistent manner.
...
This section defines a set of C# coding standards to be used by the CAPS build team to guarantee maximum legibility, reliability, re-usability and homogeneity of our code. Each section is marked *Mandatory* or *Recommended*. *Mandatory* sections will be enforced during code reviews as well as tools like *StyleCop* and *FxCop*, and code will not be considered complete until it is compliant.
...
The file name is derives from the class name.
...
* **DO** use PascalCasing for all public member, type, and namespace names consisting of multiple words.
    ...
    HtmlTag
    IOStream
**NOTE**: A special case is made for two-letter acronyms in which both letters are capitalized (for example, *IOStream*)
...
* **DO** Use `Path.Combine()` or `Path.DirectorySeparatorChar` to separate directories. If this is not possible (such as in scripting), use a forward slash `/`. Windows is more flexible than Linux in this regard.
...
xUnit.net includes many kinds of assertions – please use the most appropriate one for your test. This will make the tests much more readable and also allows the test runner to report the best possible errors (whether it's local or the CI machine). For example, these are bad:
...
#### Parallel tests
By default all unit test assemblies should run in parallel mode. Unit tests shouldn't depend on any shared state, and so should generally be runnable in parallel. If the tests fail in parallel, the first thing to do is to figure out why; do not just disable parallel tests!
