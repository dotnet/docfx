...
namespace Microsoft.Docs.Build
{
    internal class ExternalXrefMapLoader
    {
        ...
        public static IReadOnlyDictionary<string, Lazy<ExternalXrefSpec>> Load(Docset docset, FileDownloader fileDownloader)
        {
            var result = new Dictionary<string, Lazy<ExternalXrefSpec>>();

            foreach (var url in docset.Config.Xref)
            {
                using (var stream = fileDownloader.DownloadStream(url))
                {
