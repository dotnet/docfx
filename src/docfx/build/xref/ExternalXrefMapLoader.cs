...
namespace Microsoft.Docs.Build
{
    internal class ExternalXrefMapLoader
    {
        ...
        public static IReadOnlyDictionary<string, Lazy<ExternalXrefSpec>> Load(Config config, FileResolver fileResolver, ErrorLog errorLog)
        {
            ...
            {
                var result = new ConcurrentDictionary<string, Lazy<ExternalXrefSpec>>();
        ...
        private static void LoadZipFile(ConcurrentDictionary<string, Lazy<ExternalXrefSpec>> result, FilePath path, string physicalPath, ErrorLog errorLog)
        {
