...
namespace Microsoft.Docs.Build
{
    internal static class BuildPage
    {
        ...
        {
            var (errors, token) = YamlUtility.ParseWithValidation(file, context);
        ...
        {
            var (errors, token) = JsonUtility.ParseWithValidation(file, context);
            ...
            LoadSchemaDocument(Context context, List<Error> errors, JToken token, Document file, Action<Document> buildChild)
        {
            ...
            var (schemaViolationErrors, content) = JsonUtility.ToObjectWithValidation(token, schema.Type, transform: AttributeTransformer.TransformSDP(context, file, buildChild));
