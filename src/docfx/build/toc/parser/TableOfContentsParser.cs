...
namespace Microsoft.Docs.Build
{
    internal static class TableOfContentsParser
    {
        ...
        public static (List<Error> errors, List<TableOfContentsItem> items, JObject metadata, List<string> monikers) Load(Context context, Document file, ResolveContent resolveContent, ResolveHref resolveHref)
        {
            ...
            var items = inputModel?.Items?.Select(r => TableOfContentsInputItem.ToTableOfContentsModel(r, file.Docset.Monikers.AscendingComparer)).ToList();
            var fileMonikers = items?.SelectMany(r => r.Monikers).Distinct(file.Docset.Monikers.AscendingComparer).ToList();
            fileMonikers.Sort(file.Docset.Monikers.AscendingComparer);
