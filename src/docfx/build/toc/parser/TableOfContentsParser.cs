...
namespace Microsoft.Docs.Build
{
    internal static class TableOfContentsParser
    {
        ...
        private static (List<Error> errors, TableOfContentsModel tocModel) LoadTocModel(Context context, Document file, string content = null)
        {
            ...
            {
                var (errors, tocToken) = content is null ? context.Input.ReadYaml(file.FilePath) : YamlUtility.Parse(content, file.FilePath);
            ...
            {
                var (errors, tocToken) = content is null ? context.Input.ReadJson(file.FilePath) : JsonUtility.Parse(content, file.FilePath);
                var (loadErrors, toc) = LoadTocModel(tocToken);
                errors.AddRange(loadErrors);
                return (errors, toc);
            }
