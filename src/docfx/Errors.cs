...
namespace Microsoft.Docs.Build
{
    internal static class Errors
    {
        ...
        public static DocfxException RedirectionDocumentIdConflict(List<Document> redirectFromDocs, Document redirectTo)
            => new DocfxException(ReportLevel.Warning, "redirected-id-conflict", $"Multiple documents {string.Join(", ", redirectFromDocs.OrderBy(f => f.FilePath).Select(f => $"'{f}'"))} redirect to '{redirectTo}' with document id");
    }
}
