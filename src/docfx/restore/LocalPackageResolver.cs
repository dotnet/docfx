...
{
    internal class LocalPackageResolver : PackageResolver
    {
        ...
        public LocalPackageResolver(string docsetPath, PreloadConfig config, FetchOptions fetchOptions, Repository? repository)
        {
        ...
        public override bool TryResolvePackage(PackagePath package, PackageFetchOptions options, [NotNullWhen(true)] out string? path)
        {
            try
            {
        ...
        public override Package ResolvePackage2(PackagePath package, PackageFetchOptions options)
        {
            return new LocalPackage(ResolvePackage(package, options));
        }

        public override string ResolvePackage(PackagePath package, PackageFetchOptions options)
        {
            switch (package.Type)
            {
