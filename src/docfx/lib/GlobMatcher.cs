...
namespace Microsoft.Docs.Build
{
    ...
    internal class GlobMatcher
    {
        ...
        {
            Options = options;
            Raw = pattern ?? throw new ArgumentNullException(nameof(pattern));
        ...
        private bool MatchOne(string[] fileParts, GlobRegexItem[] globParts, bool matchPartialGlob)
        {
            ...
            for (int j = 0; j < globParts.Length; j++)
            {
                ...
                else
                {
                    ...
                    if (globPart.ItemType == GlobRegexItemType.GlobStar)
                    {
            ...
            for (int i = 0; i < fileParts.Length; i++)
            {
                ...
                for (int j = 0; j < globParts.Length; j++)
                {
                    ...
                    {
                        case GlobRegexItemType.GlobStar:
                            if (DisallowedMatchExists(filePart))
                            {
                                status[cur, j + 1] = false;
                            }
                            else
                            {
