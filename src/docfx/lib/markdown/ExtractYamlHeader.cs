        ...
        public static (List<Error> errors, JObject metadata) Extract(TextReader reader, Document file)
        {
            ...
            while (reader.Peek() != -1)
            {
                ...
                if (trimEnd == "---" || trimEnd == "...")
                {
                    try
                    {
                        var (yamlErrors, yamlHeaderObj) = YamlUtility.Parse(builder.ToString(), new FilePath(file));
                        ...
                        errors.Add(Errors.YamlHeaderNotObject(isArray: yamlHeaderObj is JArray, new FilePath(file)));
                    }
