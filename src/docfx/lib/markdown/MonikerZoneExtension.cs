...
namespace Microsoft.Docs.Build
{
    internal static class MonikerZoneExtension
    {
        public static MarkdownPipelineBuilder UseMonikerZone(
            this MarkdownPipelineBuilder builder, Func<SourceInfo<string>, List<string>> parseMonikerRange)
        {
            return builder.Use(document =>
            {
                document.Replace(node =>
                {
                    ...
                    {
                        monikerRangeBlock.GetAttributes().Properties.Remove(
                            new KeyValuePair<string, string>("range", monikerRangeBlock.MonikerRange));
