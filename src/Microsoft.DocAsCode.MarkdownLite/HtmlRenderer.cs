...
namespace Microsoft.DocAsCode.MarkdownLite
{
    ...
    public class HtmlRenderer
    {
        ...
        protected static StringBuffer AppendSourceInfo(StringBuffer result, IMarkdownRenderer renderer, IMarkdownToken token)
        {
            ...
            {
                result = result + " sourceFile=\"" + StringHelper.HtmlEncode(token.SourceInfo.File) + "\" sourceStartLineNumber=\"" + token.SourceInfo.LineNumber.ToString() + "\" sourceEndLineNumber=\"" + (token.SourceInfo.LineNumber + token.SourceInfo.ValidLineCount).ToString() + "\"";
            }
            return result;
        }
