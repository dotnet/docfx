    ...
    public class ModuleValidator : ValidatorBase
    {
        ...
        public override bool Validate(Dictionary<string, IValidateModel> fullItemsDict)
        {
            ...
            foreach (var item in Items)
            {
                ...
                {
                    itemValid = false;
                    LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_MetadataError, result, item.SourceRelativePath);
                }

                if (module.Achievement == null)
                {
                    LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_NoBadgeBind, file: item.SourceRelativePath);
                }
                else if (module.Achievement is string achievementUID)
                {
                    ...
                    {
                        itemValid = false;
                        LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_BadgeNotFound, achievementUID, item.SourceRelativePath);
                    }
                    else if (!(fullItemsDict[achievementUID] is AchievementValidateModel achievement) || achievement.Type != AchievementType.Badge)
                    {
                        itemValid = false;
                        LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_NonSupportedAchievementType, achievementUID, item.SourceRelativePath);
                    }
                }
                ...
                var childrenNotUnit = module.Units.Except(childrenCantFind).Where(u =>
                {
                    ...
                    {
                        itemValid = false;
                        LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_MultiParents, $"{fullItemsDict[u].Uid}, {fullItemsDict[u].Parent?.Uid}, {module.Uid}", item.SourceRelativePath);
                    }
                    else
                    {
                ...
                {
                    itemValid = false;
                    LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_ChildrenNotFound, string.Join(",", childrenCantFind), item.SourceRelativePath);
                }

                if (childrenNotUnit.Any())
                {
                    itemValid = false;
                    LearnValidationLogger.Log(LearnErrorLevel.Error, LearnErrorCode.TripleCrown_Module_NonSupportedChildrenType, string.Join(",", childrenNotUnit.Select(c => c.Uid)), item.SourceRelativePath);
                }
