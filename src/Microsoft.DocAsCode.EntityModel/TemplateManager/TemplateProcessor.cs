...
namespace Microsoft.DocAsCode.EntityModel
{
    ...
    public class TemplateProcessor : IDisposable
    {
        ...
        private void TranformHtml(DocumentBuildContext context, string transformed, string relativeModelPath, string outputPath)
        {
            ...
            if (hrefNodes != null)
                foreach (var link in hrefNodes)
                {
                    ...
                    if (link.Name == "xref")
                    {
                        UpdateXref(link, context.XRefMap, context.XRefSpecMap, s => UpdateFilePath(s, relativeModelPath));
                    }
                    else
                    {
        ...
        private static void UpdateXref(HtmlAgilityPack.HtmlNode xref, Dictionary<string, string> map, Dictionary<string, XRefSpec> xrefSpecs, Func<string, string> updater)
        {
            ...
            if (map.TryGetValue(key, out xrefValue))
            {
                ...
                xref.SetAttributeValue(attribute, xrefValue);
        ...
        private sealed class SystemAttributes
        {
            ...
            private void GetTocInfo(DocumentBuildContext context, ManifestItem item)
            {
                ...
                while (tocMap.TryGetValue(currentPath, out parentTocs) && parentTocs.Count > 0)
                {
                    ...
                    currentPath = Root + (RelativePath)currentPath;
                }
                if (rootToc != null)
                {
