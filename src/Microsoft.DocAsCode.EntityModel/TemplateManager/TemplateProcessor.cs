...
namespace Microsoft.DocAsCode.EntityModel
{
    ...
    public class TemplateProcessor : IDisposable
    {
        ...
        public void Process(DocumentBuildContext context, string outputDirectory)
        {
            ...
            foreach (var item in context.Manifest)
            {
                ...
                else
                {
                    ...
                    if (context.FileMap.TryGetValue(key, out value))
                    {
                        ...
                        extMapping[value] = defaultTemplate.Extension;
                    }
                    else
                    {
            ...
            //update XrefMap
            if (context.ExternalXRefSpec != null)
            {
                foreach (var pair in context.ExternalXRefSpec)
                {
                    string ext;
        ...
        private void TranformHtml(DocumentBuildContext context, string transformed, string relativeModelPath, string outputPath)
        {
            ...
            if (hrefNodes != null)
                foreach (var link in hrefNodes)
                {
                    ...
                    if (link.Name == "xref")
                    {
                        UpdateXref(link, context.ExternalXRefSpec, context.XRefSpecMap, s => UpdateFilePath(s, relativeModelPath));
                    }
                    else
                    {
        ...
        private static void UpdateXref(HtmlAgilityPack.HtmlNode xref, Dictionary<string, XRefSpec> map, Dictionary<string, XRefSpec> xrefSpecs, Func<string, string> updater)
        {
            string attribute = "href";
            var key = xref.GetAttributeValue(attribute, null);
            var name = xref.GetAttributeValue("name", null);
            string displayName = name ?? key;
            XRefSpec spec = null;
            xrefSpecs.TryGetValue(key, out spec);

            XRefSpec xrefValue;
            if (map.TryGetValue(key, out xrefValue))
            {
                xref.Name = "a";
                xrefValue.Href = updater(xrefValue.Href);
                if (spec != null) displayName = spec.Name; // TODO: language specific
                xref.SetAttributeValue(attribute, xrefValue.Href);
                xref.AppendChild(HtmlAgilityPack.HtmlNode.CreateNode(displayName));
            }
            else
            {
                // If xref not found, fall back to plain text
                if (spec != null)
                {
                    string fullName = null;
                    spec.TryGetValue("fullName", out fullName);
                    if (fullName != null) displayName = fullName; // TODO: language specific
                }
        ...
        private sealed class SystemAttributes
        {
            ...
            {
                Language = "csharp"; // TODO: multiple language
