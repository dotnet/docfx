...
public class MarkdownFragmentsValidationTest : TestBase
{
    ...
    public void OverwriteUnEditableTest()
    {
        ...
        Assert.AreEqual(5, warningLogs.Count);
        Assert.AreEqual(
...
""/operations/1"" in overwrite object fails to overwrite ""/operations"" for ""management.azure.com.advisor.fragmentsValidation"" because it does not match any existing item.".ReplaceLineEndings(),
            string.Join(Environment.NewLine, warningLogs.Select(x => x.Message)));
        Assert.AreEqual("14", warningLogs[2].Line);
        Assert.AreEqual("17", warningLogs[3].Line);
    }
