...
namespace Microsoft.Docs.Build
{
    public class JsonUtilityTest
    {
        ...
        public void TestObjectWithStringProperty(string input)
        {
            ...
            var (errors, value) = JsonUtility.DeserializeWithValidation<BasicClass>(json);
            Assert.Empty(errors);
  ...
  ""d"": true
        ...
        {
            var (errors, value) = JsonUtility.DeserializeWithValidation<BasicClass>("{\"B\":1}");
        ...
        public void TestBoolean()
        {
            ...
            var (errors, value) = JsonUtility.DeserializeWithValidation<object[]>(json);
            Assert.Empty(errors);
            Assert.NotNull(value);
        ...
        public void TestListOfBasicClass()
        {
            ...
            var (errors, values) = JsonUtility.DeserializeWithValidation<List<BasicClass>>(json);
            Assert.Empty(errors);
            ...
            var json = @"
{
    ""b"": ""test""
            ...
            var (errors, value) = JsonUtility.DeserializeWithValidation<ClassWithReadOnlyField>(json);
            Assert.Empty(errors);
            Assert.NotNull(value);
  ...
  ""d"": true
                ...
                json.Replace("\r\n", "\n"));
            var (errors, value) = JsonUtility.DeserializeWithValidation<ClassWithMoreMembers>(json);
        ...
        {
            var (errors, result) = JsonUtility.DeserializeWithValidation<JToken>(json.Replace('\'', '"'));
        ...
        public void TestEmptyString()
        {
            ...
            var exception = Assert.Throws<DocfxException>(() => JsonUtility.ParseWithValidation(json));
        }
...
""nestedSealedMember"": {""unknown"": 1}}]", 5, 33, ErrorLevel.Warning, "unknown-field", typeof(List<NotSealedClass>))]
        ...
        {
            var (_, token) = JsonUtility.ParseWithValidation(json);
            var (errors, result) = JsonUtility.ToObjectWithValidation(token, type);
            Assert.Collection(errors, error =>
            {
...
""mismatchField2"": ""name"",
""valueRequired"": ""a""}";
            var (errors, result) = JsonUtility.DeserializeWithValidation<ClassWithMoreMembers>(json);
...
'valueRequired': 'a'}", ErrorLevel.Error, "violate-schema", 3, 9, "numberList[1]")]
        ...
        {
            var (errors, value) = JsonUtility.DeserializeWithValidation<ClassWithMoreMembers>(json.Replace('\'', '\"'));
            Assert.Collection(errors, error =>
            {
                Assert.Equal(expectedErrorLevel, error.Level);
                Assert.Equal(expectedErrorCode, error.Code);
...
""e"": ""e""}]", typeof(List<NotSealedClass>))]
        ...
        {
            var (_, token) = JsonUtility.ParseWithValidation(json);
            var (errors, value) = JsonUtility.ToObjectWithValidation(token, type);
            Assert.Empty(errors);
        }
...
""nestedSealedMember"": {""unknown"": 1}}]";
            var (errors, value) = JsonUtility.DeserializeWithValidation<List<NotSealedClass>>(json);
            Assert.Collection(errors, error =>
            {
                Assert.Equal(ErrorLevel.Warning, error.Level);
                Assert.Equal("unknown-field", error.Code);
