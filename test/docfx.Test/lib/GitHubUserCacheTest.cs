...
namespace Microsoft.Docs.Build
{
    public class GitHubUserCacheTest
    {
        ...
        internal async Task TestCache(string name)
        {
            ...
            var users = JsonUtility.Deserialize<GitHubUser[]>(testCase.CacheUsersJson.Replace("\'", "\""), null);
            ...
            var expectedUsers = JsonUtility.Deserialize<GitHubUser[]>(testCase.ExpectedOutputCacheUsersJson.Replace("\'", "\""), null);
        ...
        private static IEnumerable<object[]> PrepareDataCore()
        {
            ...
            yield return new object[]
            {
                ...
                (Func<GitHubUserCache, Task>) ( async (cache) => await cache.GetByCommit("me2@contoso.com", "not-exist", "not-exist", "1") ),
                "[]",
                "[]",
                0,
                1
