    ...
    public static class DocfxTest
    {
            ...
            CreateDocset(TestData test, DocfxTestSpec spec)
        {
            ...
            var repos = spec.Repos
                .Select(repo => new PackageUrl(repo.Key).Remote)
                .Distinct()
            ...
            if (!File.Exists(markerPath))
            {
                foreach (var (url, commits) in spec.Repos.Reverse())
                {
                    ...
                    TestUtility.CreateGitRepository(repos[packageUrl.Remote], commits, packageUrl.Remote, packageUrl.Branch, variables);
                }
