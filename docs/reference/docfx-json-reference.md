...
Defaults to `false`. When enabled, omits **View Source** links for code with explicit generated-code evidence:

- `System.CodeDom.Compiler.GeneratedCodeAttribute` on the output symbol or its containing declaration.
- An `<auto-generated>` or `<autogenerated>` marker, optionally self-closing, at the start of a leading ordinary comment in C# or Visual Basic source. Markers in code, strings, XML documentation examples, or later comments are not used.

For source and project inputs, the attribute must belong to the selected declaration or a containing declaration. Other partial declarations remain eligible. For compiled assemblies, attributes are read from the DLL and headers from portable-PDB `EmbeddedSource` records (compressed or uncompressed). A type attribute is applied to that type and its members only when its PDB documents identify a single source document. Metadata merges partial-type attributes: when multiple documents prevent attributing that marker to a declaration, document headers can still identify generated parts; otherwise those links are retained.

This option does **not** use `Microsoft.CodeAnalysis.GeneratorAttribute`, which identifies the generator implementation, or `CompilerGeneratedAttribute`, which can also appear on ordinary auto-property accessors. It does not infer generation from an `obj` directory, a `.g.cs` suffix, or the mere presence of embedded source. No local or remote file-existence checks are made. Without a supported marker or readable embedded source, links are retained; unreadable embedded-source records produce an `InvalidEmbeddedSource` warning. Only C# and Visual Basic header comments are recognized.

This is an opt-in policy: even a marked file with a valid repository URL loses its link when enabled. APIs, API reference links, and relative XML code includes remain available. For a partial type, a non-excluded document can still provide a link. `sourceLinkExclude` is independent and can additionally exclude unmarked or otherwise unknown documents; turning this option off does not disable those explicit patterns.
  ...
  "metadata": [
    {
      "src": [{ "files": ["**/bin/Release/**/NetCord.dll"] }],
      "dest": "api",
      "excludeGeneratedSourceLinks": true,
      "sourceLinkExclude": ["**/UnmarkedGenerator/**"]
    }
  ]
}
```
