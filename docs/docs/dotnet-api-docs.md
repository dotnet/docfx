...
If a Source Link mapping covers generated files that are not available in the repository, use [`metadata.sourceLinkExclude`](../reference/docfx-json-reference.md#sourcelinkexclude) to omit their View Source links by document path, or opt into [`metadata.excludeGeneratedSourceLinks`](../reference/docfx-json-reference.md#excludegeneratedsourcelinks) to recognize explicit generated-code markers. Neither option removes generated APIs from the documentation. Both are disabled by default.

### Exclude selected View Source links

#### Automatically exclude explicitly marked generated code

To omit View Source links for marked output without maintaining a list of generator paths, set `excludeGeneratedSourceLinks` to `true` in a metadata item:
...
This recognizes output marked with `[System.CodeDom.Compiler.GeneratedCode(...)]` or a leading ordinary `// <auto-generated/>` comment (the Visual Basic equivalent uses `'`). When reading a DLL, header detection requires that source text be embedded in its portable PDB. An embedded handwritten file without such a header is not excluded. This does not check `[Generator]`, which marks the generator class rather than its output.

The option defaults to `false`, since explicitly generated files can also be checked into a repository. Enabling it omits links for recognized generated output even when its URL is valid. Unknown documents retain their links; use the explicit rules below for generators without detectable markers. See the [reference](../reference/docfx-json-reference.md#excludegeneratedsourcelinks) for partial-declaration rules and supported evidence.
...
With `excludeGeneratedSourceLinks` left at its default `false`, remove `sourceLinkExclude` or use:
