...
The YAML header is used to annotate a Markdown file with a unique identifier for that document.  It should appear at the top of the document.  Here's an example:
...
...
```
The UID is intended to be unique inside a project. If you define duplicate UID for two files, the resolve result is undetermined.
