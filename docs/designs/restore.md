...
And there is another index file `index.txt` under `%DOCFX_APPDATA_PATH%/downloads/{url-short-path}/` tracing the detail info of each `{number} file`.
...
- `{in-use}` is tracing if the `work-tree` is in use or not, if it is, it can't be operated any git operations, otherwise, we can reuse this `work-tree` during restore, fetch and checkout to new branch/commit.
