...
The user needs to set where to get the `.xrefmap.json` files in `.openpublishing.publish.config.json`.
```json
{
    "docsets_to_publish": [
        {
...
> - The base path of current building repository should be added as default.
...
> - The resolving order of the same UID from multiple resources should respect the order of `xref_query_tags`

#### Migration
For the existing repositories using DocFX v2, the xref config has been in `.openpublishing.publish.config.json`, no need to migrate.

#### Localization
The setting is located in `.openpublishing.publish.config.json`, nothing to do with docfx configuration.
